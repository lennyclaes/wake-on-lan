 ...
 * @returns A promise that returns a string with a success message
 */
function sendWake(mac: string, retries: number = 1): Promise<string> {
    console.log(retries);
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            return reject(new Error(`Failed to broadcast wake: ${err.message}`));
        }
        socket
            .on('error', (err: Error) => {
                reject(err.stack);
        ...
        const sendInterval: NodeJS.Timer = setInterval(() => {
            socket.send(magicPacket, 0, magicPacket.length, 9, '192.168.0.255', (err: Error | null) => {
                if (err) {
                    return reject(new Error(`Failed to broadcast wake: ${err.stack}`));
                }
                if (attempts >= retries) {
                    resolve(`Broadcast(s) succesfully sent to ${mac}`);
                    socket.close();
